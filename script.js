...
(function () {
  ...
  function initScientificCalculator() {
    ...
    openCasioFracDialog = function (isMixed = false) {
      ...
      if (isMixed) {
        ...
        if (btnFracInsert) btnFracInsert.textContent = 'Insert Mixed';
        if (fracInputWhole) {
        ...
        if (fracDialogTitle) fracDialogTitle.textContent = 'Fraction Entry (a/b)';
        if (btnFracInsert) btnFracInsert.textContent = 'Insert';
        if (fracInputNum) {
          fracInputNum.value = '';
          fracInputNum.focus();
        }
      }
      if (fracInputNum && isMixed) fracInputNum.value = '';
  ...
  function handleCasioAction(action, val) {
    switch (action) {
      ...
      case 'fraction': {
        ...
        // 2. If Shift was active, open Mixed Fraction entry helper
        if (isShift) {
          if (typeof openCasioFracDialog === 'function') {
            openCasioFracDialog(true);
          }
          return;
        }

        // 3. If expression is empty or ends in an operator or opening parenthesis, open Natural Fraction helper
        if (!expr || /[+\-×÷(]$/.test(expr)) {
          if (typeof openCasioFracDialog === 'function') {
            openCasioFracDialog(false);
          } else {
            sciState.expression += '(/)';
          }
          return;
        ...
        // 4. If expression already ends in fraction slash, do nothing to avoid syntax error
        if (expr.endsWith('/')) {
          return;
        }

        // 5. If preceded by a number, parenthesis, or constant, append fraction separator directly
        sciState.expression += '/';
        break;
      }
  ...
  function parseAndCompute(rawExpr, angleMode, ansVal) {
    ...
    // Support mixed fractions e.g. "1 1/2" or "2_3/4"
    str = str.replace(/(\d+)\s+(\d+)\/(\d+)/g, '($1+$2/$3)');
    str = str.replace(/(\d+)_(\d+)\/(\d+)/g, '($1+$2/$3)');
